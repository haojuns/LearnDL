...
class VectorOp(object):#lamda有map和reduce两种用法
    """
    实现向量计算操作
    """
    @staticmethod
    def dot(x, y):
        ...
        # 然后利用reduce求和
        return reduce(lambda a, b: a + b, VectorOp.element_multiply(x, y), 0.0)#lambda arguments: expression
    ...
    def element_multiply(x, y):
        ...
        return list(map(lambda x_y: x_y[0] * x_y[1], zip(x, y)))#map惰性求值，所以返回的是一个还没有计算和存储的可迭代对象，list就会让map真正求值
