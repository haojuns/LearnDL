...
class CheckpointFunction(torch.autograd.Function):
    ...
    def backward(ctx, *args):
        ...
        tensors = ctx.saved_tensors#取tensor
        ...
        # when we're done.这个模块在负责进行重新前向传播计算
        rng_devices = []
        ...
        with torch.random.fork_rng(devices=rng_devices, enabled=ctx.preserve_rng_state):
            ...
            detached_inputs = detach_variable(tuple(inputs))#为什么要分离input出来？？
        ...
        torch.autograd.backward(outputs_with_grad, args_with_grad)#计算args_with_grad列表里的张量和outputs_with_grad的梯度（grad属性）
...
def checkpoint_sequential(functions, segments, input, use_reentrant=True, **kwargs):
    ...
    segment_size = len(functions) // segments#有问题
