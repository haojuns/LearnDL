...
def LinearBackward(grad_output,input,weight):
        ...
        # 计算输入的梯度
        grad_input = torch.from_numpy(np.matmul(grad_output, weight))

        # 计算权重的梯度
        grad_weight = torch.from_numpy(np.matmul(grad_output.T, input))

        # 计算偏置的梯度
        grad_bias = torch.from_numpy(np.sum(grad_output, axis=0))
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def backward(ctx, *args):
        ...
        detached_inputs_new = (detached_inputs[0][selected_indices],)[0].detach()
        detached_inputs_new[0].requires_grad=True
        
        outputs_new = (outputs[0][selected_indices],)[0].detach()
        outputs[0].requires_grad=True
        outputs_new[0].grad_fn=outputs[0].grad_fn
        
        args_new = (args[0][selected_indices],)[0].detach()
        args_new[0].requires_grad=True'''
        
        '''start=time.time()
        grads_numpy=LinearBackward(args[0],inputs[0],ctx.run_function.weight.data)
        times_1=time.time()-start'''
        ...
        for i in range(len(outputs)):
            if torch.is_tensor(outputs[i]) and outputs[i].requires_grad:
                outputs_with_grad.append(outputs[i])#该层的输出中间激活i
                args_with_grad.append(args[i])#后层传过来的中间激活i的梯度
